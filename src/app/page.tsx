...
export default async function Home() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1480px] flex-1 flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      <section className="grid gap-5 rounded-lg border border-white/10 bg-[#101723]/88 p-5 shadow-[0_24px_70px_rgba(0,0,0,0.38)] backdrop-blur lg:grid-cols-[1fr_340px]">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="text-3xl font-semibold tracking-tight text-stone-100 sm:text-4xl">
              Living Outlands Plant Collection
            </h1>
            <p className="max-w-3xl text-sm leading-6 text-stone-400 sm:text-base">
              Private catalog by sprite ID, with origin, quantity, and images extracted from the
              client. Plants with the same name stay separated when they use different art.
            </p>
          </div>
        </div>

        <aside className="flex flex-col justify-between gap-4 rounded-lg border border-amber-400/20 bg-[linear-gradient(135deg,rgba(38,30,16,0.9),rgba(18,24,34,0.92))] p-4">
          <div className="space-y-2">
            <p className="text-xs font-semibold tracking-[0.25em] text-amber-300 uppercase">
              Session
            </p>
            <p className="text-sm leading-6 text-stone-300">Signed in as {displayName}</p>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Plants" value={catalog.summary.totalPlants.toLocaleString("en-US")} />
        <MetricCard label="Sprites" value={catalog.summary.varieties.toLocaleString("en-US")} />
        <MetricCard label="Names" value={catalog.summary.uniqueNames.toLocaleString("en-US")} />
        <MetricCard label="Origins" value={catalog.summary.uniqueOrigins.toLocaleString("en-US")} />
      </section>
